...
import { batchAddMovies } from './onboarding-actions'

interface OnboardingFlowProps {
  existingMovies: UserMovie[]
}
...
export function OnboardingFlow({ existingMovies }: OnboardingFlowProps) {
  ...
  const maxSelectable = 50 - existingCount
  const currentCount = selectedMovies.size

  useEffect(() => {
    async function fetchTopRated() {
      try {
        const res = await fetch('/api/tmdb/top-rated?pages=10')
  ...
  function toggleMovie(movie: TMDBMovie) {
    ...
    if (newSelected.has(movie.id)) {
      ...
      const milestone = MILESTONE_MESSAGES[newCount + existingCount]
      const message = milestone || FUN_MESSAGES[Math.floor(Math.random() * FUN_MESSAGES.length)]
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      await batchAddMovies(movies, existingCount)
      router.push('/my-list')
    })
  }

  const moviesToShow = showSearch && searchQuery.length >= 2 ? searchResults : topRatedMovies
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          <div>
            <div className="text-2xl font-bold">
              {currentCount + existingCount} / 50
            </div>
            <p className="text-sm text-gray-500">
              {currentCount === 0
                ? "Tap movies above to get started"
                : `${currentCount} selected this session`}
            </p>
          </div>
